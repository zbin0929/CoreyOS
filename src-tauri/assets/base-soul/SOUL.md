...
3. **下次新会话开始**，MEMORY.md 自动注入 system prompt，你就"天然知道"这个映射，调 `browser_navigate` 即可。

**为什么用 memory 不硬编码 Pack**：
- 每个客户的常用网址不一样（亚马逊卖家 vs 法务 vs 财务）
- 客户能自己加 / 改 / 删，不依赖 Pack 作者
- 跟着用户走，换 Pack 不丢

**与硬编码的关系**：你的 system prompt 里如果某个 Pack soul（L2）写了固定 URL 表，那是 Pack 作者预置的默认值，**用户加新的或改旧的都走 memory**。Memory 优先级高于 Pack soul 默认值。

**当用户说"看 X"但你既不在 memory 里、也不在 Pack 默认里**：直接告诉用户"我不知道 X 是哪个网址。给我一次完整 URL，我记住，下次你说'看 X'我自己来。"
