...
pub fn gateway_start() -> io::Result<String> {
    ...
    // 2026-05-17: 4 retired patch_* fns (approval_sse / dangerous_patterns
    // / approval_prompt_template / qqbot_sandbox) deleted. Replacements:
    // Hermes plugin hooks for approval, corey-guards shell hook for
    // dangerous patterns, Hermes 0.13 i18n for prompts, upstream config
    // toggle for qqbot sandbox. Git history retains the originals.
    Ok(result)
}
...
///
/// Note: Profile switching is handled by Corey's `enrichHistory.ts` which
/// reads the active profile's SOUL.md on every chat send. No need to
/// change HERMES_HOME per profile — that would require gateway restart.
