...
// Tests live in `hermes_config_tests.rs` so the implementation file
// stays under the project's 800-line guideline (see
// `scripts/check-file-sizes.mjs`). `#[path]` keeps the canonical
// module file at `hermes_config.rs` while letting rustc find the
// test body without a sibling `hermes_config/` directory.
#[cfg(test)]
#[path = "hermes_config_tests.rs"]
mod tests;
