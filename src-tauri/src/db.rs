...
impl Db {
    ...
    // ───────────── Skill version history (v9) ─────────────
    //
    // One row = one snapshot of a skill's body taken just before the
    // file was overwritten. Retention is capped at 50 per path —
    // enough to undo a bad editing run without unbounded DB growth.
    ...
    pub fn list_knowledge_docs(
        ...
        let rows = stmt.query_map([], |row| {
            Ok(super::ipc::knowledge::KnowledgeDoc {
                id: row.get(0)?,
                name: row.get(1)?,
                filename: row.get(2)?,
    ...
    pub fn search_knowledge_chunks(
        ...
        limit: usize,
    ) -> rusqlite::Result<Vec<super::ipc::knowledge::KnowledgeSearchHit>> {
        let conn = self.conn.lock();
        Self::ensure_knowledge_tables_inner(&conn)?;
        let mut stmt = conn.prepare(
        ...
        let mut scored: Vec<super::ipc::knowledge::KnowledgeSearchHit> = Vec::new();
        for r in rows {
...
mod tests {
    ...
    fn v9_skill_versions_snapshot_list_get_and_cap_retention() {
        ...
        // Oldest surviving is i=5 (created_at=1005); the i=0..=4
        // snapshots fell off.
