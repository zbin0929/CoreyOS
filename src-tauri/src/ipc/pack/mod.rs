...
// Glob re-export so `tauri::generate_handler!` can find both
// `pack_import_zip` / `pack_uninstall` and their `__cmd__*` shims at
// `crate::ipc::pack::*`. See same pattern in `workflow/mod.rs`.
