...
mod import;
pub use import::{import_profile_at, preview_import};

pub(super) const MANIFEST_FILENAME: &str = "caduceus-profile.json";
pub(super) const PAYLOAD_PREFIX: &str = "profile/";
...
pub(super) fn profile_dir(home: &Path, name: &str) -> PathBuf {
...
pub(super) fn read_manifest(bytes: &[u8]) -> io::Result<ProfileManifest> {
...
pub(super) fn strip_payload_prefix(path: &Path) -> Option<PathBuf> {
...
/// whole import.
pub(super) fn safe_relative(rel: &Path) -> Option<PathBuf> {
...
pub(super) fn copy_then_delete(src: &Path, dst: &Path) -> io::Result<()> {
