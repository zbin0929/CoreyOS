...
- **Tools** (always visible): Tasks, Analytics, Logs
...
Routes that survive (per N-2) but are pulled from the sidebar must be added to `DEMOTED_ROUTES` in `src/app/nav-config.ts` so the `AdvancedSection` link list stays the canonical "where do I find …" answer. As of 2026-05-06: Agents, Scheduler, Runbooks, Profiles, Voice, Compare, Terminal.
