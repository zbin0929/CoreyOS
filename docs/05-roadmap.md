...
| 6     | Orchestration core    | Multi-Hermes instances + supervisor pattern + routing + feedback loop + conversational scheduling + **channel schema fixes** + first channel e2e proven | 3–4 weeks | **Planned** (2026-04-23, expanded 2026-04-23 pm twice). T6.1 feedback loop, T6.2 multi-instance Hermes, T6.3 supervisor/worker orchestration (**audit vs Hermes native `delegate_task`**), T6.4 rules-based routing, T6.5 per-agent sandbox isolation, T6.6 conversational scheduler, T6.7 **channel schema audit + e2e verification** — now 5 days after discovering 3 of 8 channels silently broken (WhatsApp/WeCom/WeChat). See `docs/phases/phase-6-orchestration.md` + `docs/hermes-reality-check-2026-04-23.md`. |
| 7     | Agent expansion       | LangGraph adapter + skill-from-chat + long-term memory + skills importer | 2–3 weeks | **Planned** (2026-04-23, re-scoped 2026-04-23 pm). T7.1 LangGraph adapter, T7.2 skill-from-conversation distillation, T7.3 memory layer (**audit vs Hermes native FTS5+memory**), T7.4 **re-scoped** to SKILL.md + agentskills.io importer (OpenClawAdapter dropped — OpenClaw merged into Hermes per upstream). See `docs/phases/phase-7-expansion.md`. |
...
Total ~13–15 weeks solo from Phase 0 through Phase 8, sequential. Phases 2/3 can parallelize if 2 devs.
