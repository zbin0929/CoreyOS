...
| 1     | Chat core             | Real SSE chat, sessions, tool calls, attachments       | 3–4 days  | **Shipped** (2026-04-22) — T1.1–T1.7 all green; T1.5 attachments + T1.5b multimodal wire format land; T1.8 reconnect and T1.9 virtualisation deferred. See `docs/phases/phase-1-chat.md`. |
...
| 4     | Differentiators       | Multi-model compare, skill editor, trajectory, budgets, terminal | 1–2 weeks | **Shipped** (2026-04-22) — T4.1–T4.6 all green. Compare / Runbooks / Budgets / Trajectory / Terminal / Skills all land with e2e + Rust tests. Deferred: CodeMirror 6 in skills, multi-tab terminal, budget-breach chat interceptor, runbook scope filtering. See `docs/phases/phase-4-differentiators.md`. |
| 5     | Multi-agent console   | 2+ non-Hermes adapters running side-by-side            | ~1 week   | **In progress** — T5.1 + T5.2a + T5.3a + T5.5a + T5.5b + T5.5c shipped (2026-04-23). Trait polish + Claude Code **mock** + Aider **mock** + conformance suite + AgentSwitcher (read-only → active selection → capability-gated Sidebar + `adapter_id` chat routing) + **unified inbox** (DB v5 migration, per-row adapter badges, Active/All-agents scope toggle). Three adapters online end-to-end. Deferred: T5.6 cross-adapter analytics/budgets, T5.2b/T5.3b real CLIs. |
