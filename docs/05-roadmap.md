...
| 7     | Agent expansion       | MCP manager UI + skill-from-chat + Memory page GUI + Skills hub wrapper | ~1.5 weeks | **In progress** (2026-04-23 pm, 2/4 tasks shipped). **T7.3 shipped** — Memory page wraps `~/.hermes/MEMORY.md` + `USER.md` (two-tab CodeMirror editor, 256 KiB cap, UTF-8 byte-accurate capacity meter, en+zh i18n). **T7.1 shipped** — MCP server manager over `mcp_servers:` section of `~/.hermes/config.yaml`, schema verified against upstream docs before coding, stdio + URL transports, restart-gateway nudge; reachability probe + enabled toggle deferred (upstream has no enabled concept). 185 Rust (+6 across T7.1+T7.3), 65 Playwright (+4). T7.2 skill-from-chat and T7.4 Skills CLI wrapper still pending. See `docs/phases/phase-7-expansion.md`. |
