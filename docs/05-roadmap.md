...
| 7     | Agent expansion       | MCP manager UI + skill-from-chat + Memory page GUI + Skills hub wrapper | ~1.5 weeks | **In progress** (2026-04-23 pm). **T7.3 shipped** — Memory page wraps `~/.hermes/MEMORY.md` + `USER.md` (two-tab CodeMirror editor, 256 KiB cap, UTF-8 byte-accurate capacity meter, en+zh i18n); 182 Rust (+3), 63 Playwright (+2). Session-search panel deferred to T7.3b. T7.1 **MCP server manager** (replaces LangGraph adapter — Hermes already supports MCP), T7.2 skill-from-chat (writes to `~/.hermes/skills/`), T7.4 **Skills refactor** wrapping `hermes skills` CLI across 7+ hub sources still pending. See `docs/phases/phase-7-expansion.md`. |
