...
| 3     | Platform channels     | 8 channels + gateway restart + WeChat QR               | ~1 week   | **In progress** — T3.1 catalog + read-only grid shipped (2026-04-22); T3.2 inline forms + atomic .env/YAML writes + diff + restart prompt shipped (2026-04-22); T3.3 WeChat QR scaffolding shipped (2026-04-22), real iLink impl still pending; T3.4 live status probing via log-grep + 30s cache + probe/refresh buttons + live pill shipped (2026-04-22); T3.5 mobile layout pending. See `docs/phases/phase-3-channels.md`. |
