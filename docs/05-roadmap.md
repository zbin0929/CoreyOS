...
| 6     | Orchestration core    | Multi-Hermes instances + supervisor pattern + routing + feedback loop + conversational scheduling + first channel e2e proven | 3–4 weeks | **Planned** (2026-04-23, expanded 2026-04-23 pm). T6.1 feedback loop (👍/👎), T6.2 multi-instance Hermes, T6.3 supervisor/worker orchestration, T6.4 rules-based routing, T6.5 per-agent sandbox isolation, T6.6 conversational scheduler (slash command + intent-detection suggestion cards), T6.7 channel e2e verification (Telegram first, `channels_verified.json` badges). See `docs/phases/phase-6-orchestration.md`. |
| 7     | Agent expansion       | LangGraph adapter + skill-from-chat + long-term memory + OpenClaw adapter | 3–4 weeks | **Planned** (2026-04-23). T7.1 LangGraph adapter (reuse existing DAG frameworks, don't build our own), T7.2 skill-from-conversation distillation, T7.3 memory layer (vector DB + RAG), T7.4 OpenClaw integration — adapter + ClawHub skill importer; positioning statement that Corey is multi-ecosystem. See `docs/phases/phase-7-expansion.md`. |
