...
| 6     | Orchestration core    | Multi-Hermes + feedback loop + routing + per-agent sandbox + channel fixes + **Scheduler wrap** | ~2 weeks | **Planned** (2026-04-23 pm, post-audit). T6.1 feedback, T6.2 multi-instance, T6.3 **surface Hermes native `delegate_task`** (2d not 5d), T6.4 routing, T6.5 sandbox, **T6.6 DROPPED** (Hermes cron already does natural-language), T6.7 channel schema audit + e2e (5d), **T6.8 NEW: Scheduler refactor to wrap `~/.hermes/cron/jobs.json`** (2d). See `docs/phases/phase-6-orchestration.md` + `docs/10-product-audit-2026-04-23.md`. |
