...
| 5     | Multi-agent console   | 2+ non-Hermes adapters running side-by-side            | ~1 week   | **In progress** — T5.1 + T5.2a + T5.5a shipped (2026-04-23). T5.1: adapter trait polish. T5.2a: Claude Code **mock** adapter + shared conformance suite. T5.5a: read-only Topbar AgentSwitcher (`adapter_list` IPC + `useAgentsStore` + pill+dropdown with health/uptime). Next: T5.5b (active selection + capability-gated nav + unified inbox), T5.2b (real CLI), or T5.3 Aider. |
