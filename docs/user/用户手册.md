...
**最后更新**：2026-05-12 · 覆盖至 v0.2.13 · 25 个功能页 · 含 Memory 知识图谱增强
...
5. [横向能力](#横向能力)
   - [全局状态栏（StatusBar）](#全局状态栏statusbar)
   - [快捷键](#快捷键)
...
> 📍 **2026-05-06 路由审计**：以下 5 个页面已从主侧边栏移除（路由保留），现统一在 **Settings → 高级 / 实验功能** 区域以链接形式呈现：**Agents** / **Scheduler** / **Runbooks** / **Profiles** / **Voice**。所有 URL 仍可直接访问（例如 `/agents`），快捷键面板（⌘K）也仍能找到。
>
> 同时改名：**Memory** → "长期记忆"、**Knowledge** → "文档知识库"，避免概念混淆。
...
- **系统概览卡片**（右侧栏）：Gateway / Hermes / MCP / Cron 四行实时状态

#### 用法提示
- 检查清单完成后会被收起；可在 Settings 里手动重置
- 所有数字点击可跳到对应详情页
- 系统概览卡片显示各子系统在线/离线状态，异常时可直接跳转排查
...
Memory 页底部新增**实体列表面板**，展示 Hermes holographic 记忆系统自动提取的所有实体：

- **列表视图**：按关联 fact 数量排序，每个实体显示名称 + fact 计数
- **详情视图**：点击任意实体 → 展开关联的 facts 列表，每条 fact 显示：
  - **分类标签**（金色 badge）：如 `preference`、`fact`、`context`
  - **信任评分**（trust score）：0-1 分，越高越可靠
  - **内容预览**：fact 原文（最多 300 字符）
- 数据来源：Hermes `memory_store.db` 的 `entities` + `fact_entities` 表，CoreyOS 只读不写
- 最多显示 50 个实体

##### 自动记忆召回（Chat 集成）
> **新功能**（2026-05-12）

每条用户消息发送前，CoreyOS 会自动：

1. 从 Hermes 记忆数据库中用 FTS5 全文检索相关 facts
2. 将命中的 facts 注入对话 context（信任评分 ≥ 0.3 才注入）
3. 最多注入 5 条相关记忆

**用户体验**：
- 当有记忆被召回时，assistant 消息下方会显示金色标签："🧠 已召回 N 条记忆"
- 用户可直观感知 agent "记得"之前的对话内容
- 召回失败时静默跳过（graceful degradation），不影响正常聊天
...
> **v0.1.11 起：语义搜索已实现**。模型文件（约 2.1 GB）从国内 CDN 下载，存储在 `~/.hermes/models/` 下。下载完成后自动标记完整性。首次搜索时懒加载模型，加载失败自动降级为关键词检索。未安装模型时仍使用关键词匹配。
...
- Chat 系统每次发送前会做三件事：
  ...
  3. **语义检索**（模型已安装时）：向量搜索 + 关键词结果融合，精度远超纯关键词。
- 命中的内容会自动拼到 system prompt 前面，无需手动操作。
- 模型未安装时自动降级为纯关键词检索，不影响使用。
...
跨工作流的运行总览。任何工作流的当前执行 / 历史执行都集中在这里，独立于 Workflow 页面（那里管的是定义）。

#### 两个 Tab

##### Active（运行中）
- 当前 `running` / `pending` / `paused` 状态的任务
- 点 **取消** 终止运行（in-flight 步骤会跑完当前调用再退出）
- 暂停（等待审批）的任务红色提示
...
后台每 5 秒拉一次 `workflow_active_runs`：
- **侧边栏 Tasks 旁边的数字** = 当前 running + paused 任务数
- 任务**完成 / 失败 / 暂停**时弹桌面通知
- 切到任意页面都在跑（不需要打开 /tasks 才感知）

#### 与 Workflow 页面的关系

| | /workflows | /tasks |
|---|---|---|
| **职责** | 工作流定义（编辑/创建/删除） | 工作流运行（监控/取消） |
| **持久化** | `~/.hermes/workflows/*.yaml` | SQLite v12 表（带审计轨迹） |
| **入口** | 主侧边栏 | 主侧边栏 ⌘T |
...
| 组件 | 技术 | 说明 |
|---|---|---|
| 语音识别 (STT) | sherpa-onnx Zipformer bilingual (int8) | 中英双语，流式识别，延迟 < 100ms |
| 语音合成 (TTS) | sherpa-onnx + VITS MeloTTS | 进程内引擎，无需外部二进制 |
| 语音检测 (VAD) | silero-vad v5 | 自动判断说完/开始说 |

**安装本地语音包**：
1. 进入 Settings → Voice
2. 点击"下载本地语音包"（约 370 MB，含 Zipformer STT + MeloTTS + Silero VAD）
...
- 点击顶部麦克风图标开启
- 自动模式：开口就说，说完自动发送
- **流式识别**：说话时实时显示识别中的文字，说完立即提交
- 按键模式：按住 Space 说话，松开发送
- AI 回复会自动朗读，按任意键可打断

**语音参数调节**：
- 语速：Settings → Voice → 语速输入框，范围 0.5~2.0（默认 1.0）
- 在 Voice 页面底部"保存"按钮统一保存所有语音设置
...
Settings → Voice → 语速输入框，范围 0.5~2.0（默认 1.0）。调节后点"保存"生效，适用于本地和云端 TTS。
...
应用底部固定显示一行状态栏，高度与侧边栏设置行对齐，提供上下文感知信息：

| 区域 | 内容 |
|------|------|
| **左侧** | Chat 页：当前会话标题 + 消息数；其他页：页面名称 |
| **中间** | 技能数 · MCP 数 · 定时任务数（实时统计） |
| **右侧** | Corey 版本号 |

- 系统统计使用 stale-while-revalidate 缓存（30 秒），避免频繁 IPC 调用
- Chat 页切换会话时标题和消息数自动更新
