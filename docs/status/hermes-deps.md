...
| 2026-05-20 | v5.2 | **0.14** | **v0.14 升级预分析（待落地）**。993 commits, 1188 files, +142K/-45K LOC。核心变更：（1）依赖精确锁定 `>=X,<Y` → `==X.Y.Z` 防供应链攻击，`anthropic` 移入 lazy_deps。（2）新增 `tools/lazy_deps.py`（613 行）按需安装后端依赖。（3）`approval.py` `check_all_command_guards` 新增 `"status": "pending_approval"` + `"approval_pending": True` 字段（原 `approval_required` 仍保留于 `check_dangerous_command`，Corey 不检查此字符串，无影响）。（4）`prompt_builder.py` 工具强制模型列表扩展：新增 `glm`/`qwen`/`deepseek`。（5）Browser 插件化重构：browser-use/firecrawl 迁移为 plugin 发现机制（Corey 用自研 CDP，无影响）。（6）Skills Hub 新增 `browse-sh` 来源。（7）Kanban 大量改进（scheduled status/drag-to-delete/respawn guard 等）。（8）perf: agent-loop 47% 减少函数调用 + CLI 延迟加载 openai 模块。（9）`api_server.py` 零修改 → `/v1/runs` SSE 协议无变化。（10）`config_version` 仍为 23。**Corey 唯一需要改动：`HERMES_MAX_TESTED` `(0, 13)` → `(0, 14)`**。 | 待执行：`hermes_config/gateway.rs` (`MAX_TESTED` bump) |
