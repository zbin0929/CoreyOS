...
| 2026-05-20 | v5.2 | **0.14** | **v0.14.0 (2026.5.16) 升级深度分析（待落地）**。993 commits, 1188 files, +142K/-45K LOC, 63 个新文件。**架构级变更**：（A）**run_agent.py 大拆分** — 提取 `agent_init.py`(1510L)、`conversation_loop.py`(3900L)、`chat_completion_helpers.py`(893L)、`tool_executor.py`、`stream_diag.py`、`background_review.py`、`agent_runtime_helpers.py`、`system_prompt.py`、`iteration_budget.py` 共 10+ 模块。（B）**插件化架构全面落地** — Web Search 7 插件（brave_free/ddgs/searxng/exa/parallel/tavily/firecrawl）、Browser 3 插件（browser-use/browserbase/firecrawl）、Video Gen 插件、Image Gen 插件，均迁移为 `plugins/` 目录 + ABC + Registry 模式；`tools/web_providers/` 和 `tools/browser_providers/` 已删除。（C）**Lazy Deps 框架**（`tools/lazy_deps.py` 613L）— `anthropic`/`bedrock`/`edge-tts`/`elevenlabs`/`faster-whisper`/`fal-client`/`telegram`/`discord`/`slack`/`matrix`/`dingtalk`/`feishu`/`honcho`/`hindsight`/`modal`/`daytona`/`vercel` 等 20+ 后端按需安装，核心 deps 从 `>=X,<Y` 全部锁定为 `==X.Y.Z`。（D）**LSP 集成** — `agent/lsp/` 子包（8 个模块），`write_file`/`patch` 后自动跑真实 Language Server 获取语义诊断（类似 Cursor/Claude Code）。（E）**Codex App-Server Runtime** — 可选把 OpenAI/Codex 模型交给 `codex app-server` 子进程执行（JSON-RPC 2.0 over stdio），替代 Hermes 自身工具分发。（F）**OpenAI-compat Local Proxy**（`hermes_cli/proxy/`）— 透明代理 OAuth provider（xAI Grok SuperGrok、Copilot）到本地 localhost，第三方工具可直接用。（G）**Cross-session Prompt Cache** — Claude 在 Anthropic/OpenRouter/Nous Portal 获得 1h 跨会话前缀缓存，首轮输入成本降 85-90%。（H）**Per-turn File Mutation Verifier** — 检测 write_file/patch 静默失败，回复末尾追加 advisory footer。（I）**Supply-chain Advisory Checker** — 检测已安装包是否受供应链攻击影响（Mini Shai-Hulud 蠕虫事件）。（J）**Skill Bundles** — `/<bundle-name>` 一次加载多个 Skill。**新 Provider/平台**：xAI Grok OAuth(SuperGrok)、NovitaAI LLM、SimpleX Chat 网关、X(Twitter) Search 工具。**新 Skill**：osint-investigation、pinggy-tunnel、darwinian-evolver、EVM multi-chain(8链14命令)。**Kanban**：scheduled status、drag-to-delete、respawn guard、worker visibility、swarm topology、bulk reassign、stale detection、max_in_progress cap。**Config 新增字段**：`compression.protect_first_n`(默认3)、`display.file_mutation_verifier`(默认true)、`security.allow_lazy_installs`(默认true)。**对 Corey 影响**：`api_server.py` 零修改 → `/v1/runs` SSE 协议不变；`config_version` 仍 23；Skills CLI 子命令不变。**Corey 唯一需要改动：`HERMES_MAX_TESTED` `(0, 13)` → `(0, 14)`**。 | 待执行：`hermes_config/gateway.rs` (`MAX_TESTED` bump) |
