...
| 8c | **把美正 Pack 从基座源码摘出** | ✅ 2026-05-17 完成（commit `40e63c0` + `8c0d7d3`） | `git mv src-tauri/assets/skill-packs/meizheng/` → `packs/meizheng/` + `.gitignore` 加 `packs/*`（仅 README 跟踪）。基座 binary 从此只 bundle `cross_border_ecom`。测试 fixture 改为 `id: test_pack` / `pack__test__*`。客户分发：私有 zip + Settings → Packs → 导入 zip。剩 step 5（CI smoke：干净启动 `packs=0` + 导入 zip 后 `packs=1`）和 `customer.yaml.preinstall_packs:` 字段，等首次客户机器实地装时再做。 |
...
| 13 | ~~**零依赖安装包（Win+Mac）**~~ | ~~5-7d~~ | ✅ 2026-05-22 完成：PyInstaller 打包 hermes-standalone（~133MB），内嵌到 Corey.app/Contents/Resources/ 和 Corey.exe 同目录。`resolve_hermes_binary()` 优先查找内嵌二进制。CI workflow 自动构建。 |
