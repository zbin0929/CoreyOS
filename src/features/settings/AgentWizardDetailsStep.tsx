...
/**
 * Step 2 — fill in the agent's identity, credentials, and model.
 *
 * The form has two modes:
 * - **Fresh provider**: user typed an API key + picked a model. We
 *   first persist an `LlmProfile` (so the credentials land in the
 *   reusable library) and then create a `HermesInstance` linked to it.
 * - **Linked profile**: user clicked an existing-profile card in
 *   Step 1 (or selected one from the picker dropdown here). Skip the
 *   credentials/model fields entirely and create only the agent row,
 *   carrying the profile's id forward.
 *
 * Validation mirrors the Rust-side `validate_id` so the Save button
 * disables for the same reasons the backend would reject.
 */
export function DetailsStep({
  ...
  existingIds: string[];
  /**
   * When set, the form opens with this profile already attached —
   * used by Step 1's "use existing LLM" cards so the user lands
   * straight on "just name it" without re-seeing the picker.
   */
  initialSelectedProfile?: LlmProfile | null;
  onBack: () => void;
  ...
  const [selectedProfileId, setSelectedProfileId] = useState<string | null>(
    initialSelectedProfile?.id ?? null,
  );
  ...
  useEffect(() => {
    llmProfileList()
      .then((r) => {
        ...
        // If the wizard entered DetailsStep with a preselected profile
        // (user clicked an "existing LLM" card in Step 1), keep the
        // selection even after the list refreshes — findById would
        // otherwise drop to null on the first render post-fetch.
      })
            ...
            <label className="flex flex-col gap-1 text-xs text-fg-muted">
              ...
              {idError ? (
                <span
                  ...
                  data-testid="agent-wizard-id-error"
                >
                  {idError}
                </span>
              ) : duplicateId ? (
                <span
                  className="text-[10px] text-danger"
                  data-testid="agent-wizard-id-dup"
                >
      ...
      {showProviderFields && (
        ...
        >
          ...
          {probeError && (
            <div
              className="text-[10px] text-danger"
              data-testid="agent-wizard-probe-error"
            >
              {probeError}
            </div>
          )}
