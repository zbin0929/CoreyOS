...
4. **FedEx Ground 分区月度更新** — 从 FedEx Rate Tools 下载分区数据并上传到美正OS（需解决网络访问问题）
...
| `scripts/download_usps_zones.py` | 下载 USPS 分区（纯 HTTP API） |
| `scripts/upload_zones_meizheng.py` | 上传分区到美正OS（支持多承运商） |
...
| UPS 分区更新 | `workflows/update-ups-zones.yaml` | 每月1号 02:00 |
| USPS 分区更新 | `workflows/update-usps-zones.yaml` | 每月1号 03:00 |
